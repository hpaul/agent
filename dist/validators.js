...
export const vReasoningDetails = v.array(v.union(vReasoningPart, v.object({
    type: v.literal("text"),
    text: v.string(),
    signature: v.optional(v.string()),
}), v.object({ type: v.literal("redacted"), data: v.string() })));
export const vSourcePart = v.union(v.object({
...
const vToolResultContent = v.array(v.union(v.object({ type: v.literal("text"), text: v.string() }), v.object({
    type: v.literal("image"),
    data: v.string(),
    mimeType: v.optional(v.string()),
})));
export const vToolResultOutput = v.union(v.object({ type: v.literal("text"), value: v.string(), providerOptions }), v.object({ type: v.literal("json"), value: v.any(), providerOptions }), v.object({
    ...
    value: v.array(v.union(v.object({
        type: v.literal("text"),
        text: v.string(),
        providerOptions,
    }), 
...
export const vAssistantContent = v.union(v.string(), v.array(v.union(vTextPart, vFilePart, vReasoningPart, vRedactedReasoningPart, vToolCallPart, vToolResultPart, vSourcePart, vToolApprovalRequest)));
...
export const vSource = v.union(v.object({
...
export const vLanguageModelCallWarning = v.union(v.object({
...
export const vStreamArgs = v.optional(v.union(v.object({ kind: v.literal("list"), startOrder: v.optional(v.number()) }), v.object({ kind: v.literal("deltas"), cursors: v.array(vStreamCursor) })));
