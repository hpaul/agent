...
import { getVectorIdInfo, validateVectorDimension, vVectorId, } from "./vector/tables.js";
...
export const getMessagesByIds = query({
    args: { messageIds: v.array(v.id("messages")) },
    handler: async (ctx, args) => {
        return (await Promise.all(args.messageIds.map((id) => ctx.db.get("messages", id)))).map((m) => (m ? publicMessage(m) : null));
    },
...
export const _fetchSearchMessages = internalQuery({
    ...
    returns: v.array(vMessageDoc),
    handler: async (ctx, args) => {
        const beforeMessage = args.beforeMessageId &&
...
export const textSearch = query({
    ...
    handler: async (ctx, args) => {
        assert(args.searchAllMessagesForUserId || args.threadId, "Specify userId or threadId");
        const targetMessage = args.targetMessageId &&
